...
Get-ChildItem -LiteralPath $current -Filter '*.md' -File | ForEach-Object {
  ...
  $inactiveDays = ($today - $_.LastWriteTime.Date).Days
  $meta = Get-Meta -Path $_.FullName
  $done = ($meta.Status -eq 'done')

  $shouldMove = $false
  if ($done -and $age -ge $DoneGraceDays) {
    $shouldMove = $true
  } elseif (-not $done -and $inactiveDays -ge $InactivityDays) {
    $shouldMove = $true
  }
  ...
  if ($shouldMove) {
    ...
    $moved++
    Write-Verbose "Archived $($_.Name) (status=$($meta.Status), age=$age, inactive=$inactiveDays)"
  }
}
...
foreach ($section in @(
  ...
  foreach ($f in $files) {
    ...
    if ($m.Status -and $section.Label -ne 'Core') { $extra += "status: $($m.Status)" }
